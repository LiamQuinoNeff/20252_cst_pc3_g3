...
function setupTimeControls() {
  ...
  const fast = document.getElementById('time-fast');   // 5x
  const max = document.getElementById('time-max');   // 10x
  const ultra = document.getElementById('time-ultra'); // 100x

  const buttons = [slow, half, normal, fast, max, ultra];

  function setScale(scale, activeBtn) {
    timeScale = scale;
    buttons.forEach((btn) => {
      if (btn) {
  ...
  if (fast) fast.addEventListener('click', () => setScale(5.0, fast));
  if (max) max.addEventListener('click', () => setScale(10.0, max));
  if (ultra) ultra.addEventListener('click', () => setScale(100.0, ultra));
...
function animate() {
  ...
  // Movimiento de blobs: la velocidad visual depende de `speed` y del timeScale.
  // Usamos una curva SUAVE para que 5x y 100x se noten, pero sin pasos gigantes
  // que rompan la fluidez (a 100x ≈ ~10x más rápido, aún animado en varios frames).
  const baseMoveSpeed = 2.6; // unidades de escena por segundo para speed ≈ 1 en 1x
  const ts = Math.max(0.1, Math.min(timeScale, 100));
  const movementScale = Math.pow(ts, 0.5); // raíz cuadrada: 1x->1, 5x->~2.2, 10x->~3.1, 100x->10
  ...
  for (const jid in creatures) {
    ...
    // Mapear speed (0.3–2.5 aprox.) a un factor visual no lineal.
    // Esto rompe la duración constante A→B: los rápidos llegan antes.
    const minSpeed = 0.3;
    const maxSpeed = 2.5;
    const s = Math.max(minSpeed, Math.min(maxSpeed, creatureSpeed));
    const visualFactor = Math.pow(s, 1.7); // p>1 => más diferencia entre lentos y rápidos
